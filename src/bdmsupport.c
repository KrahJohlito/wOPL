...
{
    if (pathParseDeviceIndex(path, "usb", NULL))
        return BDM_TYPE_USB;

    if (pathParseDeviceIndex(path, "ilink", NULL))
        return BDM_TYPE_ILINK;

    if (pathParseDeviceIndex(path, "mx4sio", NULL))
        return BDM_TYPE_SDC;

    if (pathParseDeviceIndex(path, "ata", NULL))
        return BDM_TYPE_ATA;
