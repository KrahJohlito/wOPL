...
{
    if (gPadEmuSource == SETTINGS_GLOBAL) {
        gGlobalGameCfg.pademu_enable = EnablePadEmu;
...
int guiGameSaveConfig(per_game_cfg_t *pg, item_list_t *support)
{
    ...
    if (gGSMSource == SETTINGS_PERGAME) {
        ...
        pg->gsm_enable = EnableGSM;
        pg->gsm_vmode = GSMVMode;
        pg->gsm_xoffset = GSMXOffset;
        pg->gsm_yoffset = GSMYOffset;
        pg->gsm_fieldfix = GSMFIELDFix;
    } else {
        pg->gsm_source = pg->gsm_enable = pg->gsm_vmode = pg->gsm_xoffset = pg->gsm_yoffset = pg->gsm_fieldfix = 0;
        gGlobalGameCfg.gsm_enable = EnableGSM;
        gGlobalGameCfg.gsm_vmode = GSMVMode;
        gGlobalGameCfg.gsm_xoffset = GSMXOffset;
        gGlobalGameCfg.gsm_yoffset = GSMYOffset;
        gGlobalGameCfg.gsm_fieldfix = GSMFIELDFix;
    }
#endif
    ...
    if (gCheatSource == SETTINGS_PERGAME) {
        ...
        gGlobalGameCfg.cheat_enable = EnableCheat;
...
static void guiGameLoadGSMConfig(const per_game_cfg_t *pg)
{
    ...
    if (pg && pg->gsm_source == SETTINGS_PERGAME) {
        ...
        EnableGSM = pg->gsm_enable;
        GSMVMode = pg->gsm_vmode;
        GSMXOffset = pg->gsm_xoffset;
        GSMYOffset = pg->gsm_yoffset;
        GSMFIELDFix = pg->gsm_fieldfix;
    }
...
static void guiGameLoadPadEmuConfig(const per_game_cfg_t *pg)
{
    EnablePadEmu = gGlobalGameCfg.pademu_enable;
